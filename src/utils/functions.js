...
const createInitialData = async () => {
    ...
    await Promise.all(
        data.products[0]["Tablets"].map(async (tablet) => {
            await axios.post(`/tablet/createTablet`, tablet)
        })
    )
    ...
    await Promise.all(
        data.products[1]["Laptops"].map(async (laptop) => {
            await axios.post(`/laptop/createLaptop`, laptop)
        })
    )
     //CREATE CELLPHONE
 await Promise.all(
    data.products[2]["CellPhone"].map(async (cellphone) => {
        await axios.post(`/cellPhone/createCellPhone`, cellphone)
    })
)
    ...
    await Promise.all(
        data.products[3]["Television"].map(async(tv) =>{
            await axios.post(`/tv/createTelevision`, tv)
        })
    )
}
